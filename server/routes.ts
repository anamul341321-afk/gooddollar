...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Submit key
  app.post("/api/submit-key", async (req: Request, res: Response) => {
    if (!req.session.userId) {
      return res.status(401).json({ message: "Unauthorized" });
    }
    try {
  ...
  app.post("/api/withdraw", async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (amount < 50) return res.status(400).json({ message: "Minimum withdrawal is 50 TK" });
  ...
  // Check duplicates
  app.post("/api/check-duplicates", async (req: Request, res: Response) => {
    const { numbers } = req.body;
    if (!Array.isArray(numbers)) return res.json({ duplicates: [] });
    const existing = await storage.getExistingSubmittedPhoneNumbers();
    const duplicates = numbers.filter((n: string) => existing.includes(n));
    return res.json({ duplicates });
  });

  // Lookup users
  app.post("/api/lookup-users", async (req: Request, res: Response) => {
    const { numbers } = req.body;
    if (!Array.isArray(numbers)) return res.json([]);
  ...
  app.post("/api/user/payment-feedback", async (req: Request, res: Response) => {
  ...
  app.post("/api/add-key", async (req: Request, res: Response) => {
  ...
  // Pool stats (public)
  app.get("/api/pool-stats", async (_req: Request, res: Response) => {
  ...
  app.post("/api/admin/users/:id/toggle-block", requireAdmin, async (req: Request, res: Response) => {
  ...
  app.post("/api/admin/withdrawals/:id/status", requireAdmin, async (req: Request, res: Response) => {
  ...
  app.get("/api/admin/verification-pool", requireAdmin, async (_req: Request, res: Response) => {
    const pool = await storage.getVerificationPool();
    return res.json(pool);
  });

  app.delete("/api/admin/verification-pool/:id", requireAdmin, async (req: Request, res: Response) => {
    await storage.deleteVerificationKey(parseInt(req.params.id));
    return res.json({ success: true });
  });

  app.post("/api/admin/verification-pool/delete-used", requireAdmin, async (_req: Request, res: Response) => {
    const count = await storage.deleteUsedVerificationKeys();
    return res.json({ success: true, count });
  });

  app.get("/api/admin/submitted-numbers", requireAdmin, async (_req: Request, res: Response) => {
    const numbers = await storage.getSubmittedNumbers();
    return res.json(numbers);
  });

  app.post("/api/admin/submit-numbers", requireAdmin, async (req: Request, res: Response) => {
  ...
  app.delete("/api/admin/submitted-numbers/:id", requireAdmin, async (req: Request, res: Response) => {
    await storage.deleteSubmittedNumber(parseInt(req.params.id));
    return res.json({ success: true });
  });

  app.post("/api/admin/submitted-numbers/clear", requireAdmin, async (_req: Request, res: Response) => {
    await storage.clearAllSubmittedNumbers();
    return res.json({ success: true });
  });

  app.get("/api/admin/reset-history", requireAdmin, async (_req: Request, res: Response) => {
    const history = await storage.getResetHistory();
    return res.json(history);
  });

  app.post("/api/admin/batch-reset", requireAdmin, async (req: Request, res: Response) => {
    const { numbers, submittedBy, paymentNumber, paymentMethod } = req.body;
